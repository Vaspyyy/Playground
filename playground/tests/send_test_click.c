...
static void global(void *data, struct wl_registry *registry, uint32_t name,
    ...
    fprintf(stderr,"Wayland global: %s\n",interface);
...
int main(int argc,char **argv) {
    struct wl_display *display=wl_display_connect(getenv("WAYLAND_DISPLAY"));
    if (!display || argc!=3) return 1;
    struct wl_registry *registry=wl_display_get_registry(display);
    const struct wl_registry_listener listener={global,removed};
    wl_registry_add_listener(registry,&listener,NULL);
    if (wl_display_roundtrip(display)<0) { perror("Wayland roundtrip"); return 3; }
    if (!input) return 2;
